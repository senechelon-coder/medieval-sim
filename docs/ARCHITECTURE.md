...
**Phase 1 — Time & character foundation**
`SimDate`, `TimeManager` (pause/speed/day-month-year signals), `Character` data model, `WorldState` registry, `GameData` loading a minimal set of `.tres` (a few occupations, a handful of names). Character creation flow (name/sex/birth) produces a Tier-0 player character living in one seed `Settlement`. Character screen shows stats/age updating as days pass. `SaveManager` can save/load this minimal state. **Exit criterion:** start a character, watch it age day-by-day with pause/speed controls, save and reload without state loss.

**Phase 2 — Local world & events**
