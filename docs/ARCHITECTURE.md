...
**Phase 4 — Travel, trade & piracy — EXIT CRITERION MET, DEEPER CONTENT DEFERRED**
`travel_sim.gd` implemented: multi-day travel between settlements as a day-by-day event sequence (bandits/pirates/inn/weather/trader drawn from `TravelEventData`, distance computed via province-hop pathfinding), arriving updates `location_id` and advances the calendar via `TimeManager.advance_days()`. Background merchant supply movement (`WorldState._simulate_background_trade`) and per-settlement regional pricing already existed from Phase 2/3 and now actually matter since the player can reach them. Danger encounters (bandits on land, pirates if the player is a Ship Owner) resolve automatically without a player prompt, with evade/fight/robbed outcomes. **Exit criterion met:** travel from settlement A to B takes multiple simulated days (verified: Medina → Mecca in 3 days, calendar date advances correctly), trade can produce a buy-low/sell-high profit (regional prices already differ, now reachable), background trade changes local supply independent of the player, and pirate/bandit activity disrupts routes without player involvement.
Not yet built (deferred, beyond the exit bar): `trade_route_sim.gd`/`piracy_sim.gd` as named background-simulation systems, joining/leading a caravan, working aboard a merchant vessel, joining a pirate crew, becoming a captain, and raiding/hunting other ships — these are deeper Phase 4+ content, not required by the exit criterion above.

**Phase 5 — Minimal battle & wider-world independence — EXIT CRITERION MET**
`War`/`Army` models implemented (`core/models/war.gd`, `core/models/army.gd`), driven by `core/war/war_sim.gd`. A war against a historically-grounded rival (Rashidun↔Byzantine, Byzantine↔Sasanian, Sasanian↔Rashidun) is declared once the player is an adult, and its `war_score` advances every year in the background — independent of the player — with a resolution (victory/defeat/stalemate) after enough score swing or enough years. Recruitment is a data-driven event (`data/events/war_event_data.gd`, higher odds for the Soldier occupation) offering a real join-or-decline choice. Joining triggers a scripted battle (`data/events/battle_event_data.gd`): a `BattleOverlay` steps through three timestamped narrated beats, each with two choices affecting health/morale, and a final outcome (valorous/survived/wounded) computed from accumulated morale, the player's army strength/morale, and RNG, feeding a war-score delta back into the active war and rewarding wealth/standing. **Exit criterion met:** the full vertical slice (create character → age → live in settlement → receive events → work an occupation → travel → world events happen independent of the player, verified via the war advancing in the background whether or not the player has joined → get called to a small battle → participate via text choices → outcome recorded) is verified end-to-end, including a save/load round-trip for the new `wars`/`armies` state.
Not yet built (deferred, beyond the exit bar): NPC king death/succession as an alternative independent-world-event example, and any deeper war-score effects on the map/kingdoms (no territory actually changes hands yet). Battle variety (per-rival flavor, randomized beat combinations) was added after this phase's initial completion — see `data/events/battle_event_data.gd`.

### Visual QA Track — Phase 4: Cohesive Mobile Life Interface

**Status: documented for reconstruction; not present in the restored GitHub baseline.** This visual phase records the step-by-step direction established during visual QA. It changes presentation only: no simulation rules, Age Up behavior, event outcomes, saving, or navigation logic may be altered.

1. **Preserve the established identity.** Keep the dark parchment/black foundation, antique-gold borders, serif typography, ornamental corner frame, and restrained historical tone. Improve contrast and hierarchy without introducing a different visual style.
2. **Faction selection clarity.** Give Byzantine, Sasanian, and Rashidun selections colors associated with their realms instead of showing every selection in green. Keep the map dimensions fixed when changing factions so the image, hierarchy, and controls never jump or stretch. Use a green status dot only beside the word `AVAILABLE`.
3. **Compact top status bar.** Place `WORLDLY LIFE` on the left and the year, wealth, and current standing/status on the right in one narrow framed strip. Text must remain readable at the portrait reference size.
4. **Character identity card.** Use one strong horizontal card with the portrait on the left. On the right, show the character name prominently, followed by sex, age, homeland, and current location in smaller organized lines. Do not leave a large decorative void above the card.
5. **Location strip.** Place a shallow realm/location image directly below the identity card, with a dark readability overlay and a concise location/realm caption. The artwork uses a stable crop and never changes the layout height.
6. **Dense statistics panel.** Present age, health, wealth, social standing, trait, and occupation in a compact two-row panel. Health retains a readable bar. Values align consistently and do not consume the middle of the screen.
7. **Life Events panel.** Keep Life Events immediately below the statistics. Events remain scrollable and chronological, preserve all existing event data, and use restrained color to distinguish work, family, danger, and ordinary life. Avoid stacking content until the Age Up control is pushed off-screen.
8. **Primary Age Up control.** Use a wide rectangular antique-gold `AGE UP` button directly under Life Events. It remains the clearest gameplay action and preserves its existing logic exactly.
9. **Working bottom navigation.** Keep five compact buttons—`LIFE`, `WORLD`, `ACTIVITIES`, `CHARACTER`, and `MORE`—in one row. Each must be comfortably tappable, visibly indicate the selected section, and retain its existing connection. Verify the Life button returns from overlays correctly.
10. **Overlay consistency.** World, Activities, Character, More/Pause, decisions, milestones, recruitment, and battle overlays reuse the same frame, panel, typography, spacing, and button language. Overlay work must not force changes to the main Life screen geometry.
11. **Responsive portrait composition.** Use Godot containers and responsive sizing around the portrait reference rather than scattered fixed offsets. The complete dashboard should read as one centered, intentional composition with balanced breathing room—neither tiny in the middle nor stretched edge-to-edge.
12. **Visual QA gate.** At each step, run the game at the portrait reference resolution and capture the actual rendered screen. Check text legibility, stable image crops, button connections, content overflow, event scrolling, Age Up accessibility, and navigation. Obtain visual approval before moving to the next screen.

**Approved Life-screen reference composition:** compact top status bar → character identity card → shallow location strip → compact statistics → Life Events → wide rectangular Age Up → five-button navigation. This is the reconstruction target if the visual work is resumed.

**Exit criterion:** all listed screens share one coherent historical mobile design; the Life screen matches the approved compact composition; every navigation and Age Up control works; faction switching causes no layout movement; and the complete visual pass introduces no gameplay or save-data changes.
